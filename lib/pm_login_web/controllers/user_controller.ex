      ...
      # current_user = Login.get_user!(current_id)
        cond do
          ...
          Login.is_active_client?(conn) ->
            LiveView.Controller.live_render(conn, PmLoginWeb.User.ActiveClient2IndexLive, session: %{"current_user" => Login.get_curr_user(conn),"curr_user_id" => Login.get_curr_user(conn).id}, router: PmLoginWeb.Router)
