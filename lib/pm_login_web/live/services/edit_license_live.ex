defmodule PmLoginWeb.Services.EditLicenseLive do
  use Phoenix.LiveView
  alias PmLogin.Services
  alias PmLogin.Services.Company

  def mount(_params, %{"curr_user_id"=>curr_user_id, "license" => license, "changeset" => changeset}, socket) do
    Services.subscribe()

    {:ok,
       socket
       |> assign(companies: Enum.map(Services.list_companies, fn %Company{} = c -> {c.name, c.id} end))
       |> assign(license: license, changeset: changeset)
  ...
  def render(assigns) do
   PmLoginWeb.LicenseView.render("edit.html", assigns)
  end
