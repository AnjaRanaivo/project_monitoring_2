defmodule PmLoginWeb.Services.LicenseLive do
  use Phoenix.LiveView
  alias PmLogin.Services
  alias PmLoginWeb.LiveComponent.ModalLive

  def mount(_params, %{"curr_user_id"=>curr_user_id, "licenses" => licenses}, socket) do
    Services.subscribe()

    {:ok,
       socket
       |> assign(show_modal: false, service_id: nil,curr_user_id: curr_user_id,show_notif: false, notifs: Services.list_my_notifications_with_limit(curr_user_id, 4), licenses: licenses),
  ...
  def handle_info({Services, [_, :deleted], _}, socket) do
    ...
    {:noreply, socket |> assign(licenses: licenses)}
  end
  ...
  def render(assigns) do
   PmLoginWeb.LicenseView.render("index.html", assigns)
  end
  ...
  def handle_info(
      ...
      license = Services.get_license!(service_id)
      Services.delete_license(license)
