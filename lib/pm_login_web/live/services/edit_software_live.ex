defmodule PmLoginWeb.Services.EditSoftwareLive do
  use Phoenix.LiveView
  alias PmLogin.Services
  alias PmLogin.Services.Company

  def mount(_params, %{"curr_user_id"=>curr_user_id, "software" => software, "changeset" => changeset}, socket) do
    Services.subscribe()

    {:ok,
       socket
       |> assign(companies: Enum.map(Services.list_companies, fn %Company{} = c -> {c.name, c.id} end))
       |> assign(software: software, changeset: changeset)
  ...
  def render(assigns) do
   PmLoginWeb.SoftwareView.render("edit.html", assigns)
  end
