defmodule PmLoginWeb.User.EditLive do
  ...
  def mount(_params, %{"curr_user_id" => curr_user_id,"user"=>user}, socket) do
    ...
    functions = Login.list_functions
    str_functions = Enum.map(functions, fn (%Function{} = f)  -> {f.title, f.id} end)
    str_rights = Enum.map(rights, fn (%Right{} = r)  -> {r.title, r.id} end)

    {:ok,
       socket
       |> assign(changeset: changeset, rights: rights, str_rights: str_rights, str_functions: str_functions,user: user, curr_user_id: curr_user_id, show_notif: false, notifs: Services.list_my_notifications_with_limit(curr_user_id, 4)),
